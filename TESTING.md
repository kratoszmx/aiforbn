...
Current docs profile, with the verified interpreter:

```sh
conda run -n quant python main.py --verify-agent-contract
conda run -n quant python main.py --dry-run
conda run -n quant python -m pytest -q src/tests/test_main.py src/tests/test_public_surfaces.py src/runtime/tests/test_agent_state.py src/runtime/tests/test_io_utils.py
```
...
| `src/torch_models/tests` | Small deterministic sklearn-style fit/predict, invalid-input, device-policy and ensemble checks |
...
- The three manifest-owned pytest command target sets require at least one passed non-xfail test call. All-skipped/all-xfailed exit-0 runs fail this guard; partial target selection and collect-only retain native pytest behavior. Review skip reasons rather than treating collection as execution.
