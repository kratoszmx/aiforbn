...
- Runtime reuses the stable `myutils` filesystem and JSON APIs behind project-specific path, identity, and human-document guards. Dependency availability is proven by timeout-bounded isolated imports whose successes are cached only for an opaque import-environment, owner-module, and manifest-owned consumer-target/symbol identity; ordered preloads mirror actual project import context. Project-specific artifact, dependency-contract, and AST semantics remain local.
...
- emitted architecture/docs focused profile: 566 passed;
- cache-disabled collection/full `src` suite: 1121 collected, 1121 passed;
