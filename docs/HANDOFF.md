...
- Runtime reuses the stable `myutils` filesystem and JSON APIs behind project-specific path, identity, and human-document guards. Dependency availability is proven by timeout-bounded isolated imports whose successes are cached only for an opaque import-environment, owner-module, and manifest-owned consumer-target/symbol identity; production root-symbol, static descendant target/symbol, literal-dynamic descendant target, and identity-resolved `_bind_missing` attribute imports must match that contract, and ambiguous attributes plus descendant wildcards fail closed. Ordered preloads mirror actual project import context. Project-specific artifact, dependency-contract, and AST semantics remain local.
- Deferred SHA-256 adoption: `src/runtime/io_utils.py::_file_sha256` and its two calls in `_build_published_output_digests` / `_assess_published_outputs` are exact-compatible for their current `Path` inputs with `PYTHONPATH=/Users/zmx/Projects/myutils/file_utils` plus `from filesystem import sha256_file`. The shared API was introduced by myutils `145af0f40143c8d1dbd51140276915cb6fc4c56e` and revalidated at `c430bf3c8e3688e493d472b5a4aade57656d6d7b`. Production migration is intentionally deferred to an aiforbn-owned change; retain the existing path/provenance policy and validate with `python3 main.py --verify-agent-contract`, `python3 -m pytest -q src/runtime/tests/test_io_utils.py`, and the applicable emitted full profile.
- Deferred deterministic JSON-digest adoption: `src/runtime/io_utils.py::_canonical_json_sha256` and its four calls are byte-compatible with `PYTHONPATH=/Users/zmx/Projects/myutils/file_utils` plus `from json_io import sha256_json` only when the caller keeps `make_json_safe(payload)` before the shared call. Both contracts use sorted keys, compact `(",", ":")` separators, retained Unicode, UTF-8, SHA-256, and stdlib `allow_nan=True`; myutils does not perform the project-owned preprocessing. The API was introduced by myutils `10568f6cef800ec0dce1178210ce17b8703bfd44`. Production migration is intentionally deferred to an aiforbn-owned change; validate it with `python3 main.py --verify-agent-contract`, `python3 -m pytest -q src/runtime/tests/test_io_utils.py`, and the applicable emitted profile.
...
- emitted architecture/docs focused profile: 595 passed;
- cache-disabled collection/full `src` suite: 1150 collected, 1150 passed;
