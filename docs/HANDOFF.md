...
- Shared SHA-256 adoption is complete: `src/runtime/io_utils.py` directly imports `filesystem.sha256_file` for publication and assessment after local path guards. The duplicate `_file_sha256` implementation is removed.
- Shared JSON-digest adoption is complete: provenance calls `json_io.sha256_json(make_json_safe(payload))` directly. Project normalization remains explicit; sorted compact UTF-8 bytes and stdlib non-finite-number behavior are preserved. `_canonical_json_sha256` is removed. `test_shared_digests_preserve_normalized_provenance` checks the previous byte format and both provenance paths; validate through the `module_logic_edit` profile in `TESTING.md`.
