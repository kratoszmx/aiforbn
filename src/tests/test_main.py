...
def test_run_dry_run_validates_fast_smoke_path(monkeypatch, capsys):
    spec = spec_from_file_location('main_module_under_test', ROOT / 'main.py')
    main_module = module_from_spec(spec)
    assert spec is not None and spec.loader is not None
    spec.loader.exec_module(main_module)
    ...
    feature_tables = {
        'basic_formula_composition': pd.DataFrame({'formula': ['BN'], 'target': [5.2]}),
        'matminer_composition': pd.DataFrame({'formula': ['BN'], 'target': [5.2]}),
    }
    candidate_df = pd.DataFrame({'formula': ['BN', 'AlBN']})

    monkeypatch.setattr(main_module, 'clear_project_cache', lambda path: calls.append('clear_project_cache'))
    monkeypatch.setattr(main_module, 'load_config', lambda path: calls.append('load_config') or dry_run_cfg)
    monkeypatch.setattr(main_module, 'ensure_runtime_dirs', lambda cfg: calls.append('ensure_runtime_dirs'))
    monkeypatch.setattr(
        main_module,
        'generate_bn_candidates',
    ...
    monkeypatch.setattr(
        main_module,
        'make_model',
        lambda cfg, model_type=None: calls.append(f'make_model:{model_type}') or object(),
    )

    report = main_module.run_dry_run()
    ...
    assert calls == [
        ...
        'generate_bn_candidates',
        'build_feature_tables',
        'make_model:hist_gradient_boosting',
        'make_model:linear_regression',
        'make_model:dummy_mean',
    ]
    assert report['candidate_row_count'] == 2
    assert report['dry_run_dataset_row_count'] == 3
    assert report['configured_feature_sets'] == ['basic_formula_composition', 'matminer_composition']
    assert report['available_feature_sets'] == ['basic_formula_composition', 'matminer_composition']
    assert report['screening_feature_sets'] == ['basic_formula_composition', 'matminer_composition']
    assert report['configured_model_types'] == ['hist_gradient_boosting', 'linear_regression']
    assert report['benchmark_baselines'] == ['dummy_mean']
    assert report['model_init_status'] == {
