...
- `build_structure_first_pass_execution_artifacts(...)`
  - Build the deterministic first-pass structure-execution artifacts; builder and writer share finite candidate/variant status authorities, one edit-plan projection contract, raw-source-to-final-structure identity checks, atom/metadata/CIF identity checks, evidence-compatible relaxation/final-state derivation, canonical variant ordering, and the ten-field selected projection contract.
...
- `test_reporting.py` locks artifact publication, provenance, finite candidate/variant structure-execution status vocabularies, raw-source/edit-plan/final-structure identity, atom/metadata/CIF identity, evidence-compatible variant states, role-schema preflight, repeat-run cleanup, and failure-order behavior.
