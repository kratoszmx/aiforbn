...
- `save_metrics_and_predictions(...)`
  ...
  - Reject nonempty structure-execution summary or variants tables that lack their canonical role column, or whose candidate/variant membership, counts, statuses, geometry results, or selected IDs disagree with the canonical payload/frame relationship, before mutation; zero-variant candidates cannot claim the post-attempt `executed` or `no_successful_variant` states or a successful selected-final status.
