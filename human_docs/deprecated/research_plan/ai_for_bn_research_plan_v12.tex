...
\begin{itemize}[leftmargin=*]
  ...
  \]
  Here $d_t=\domainf(c)$ for formula-stage ranking and $d_t=\domains(c,s)$ after structure hypotheses are available. All component scores are normalized to $[0,1]$, with $w_j\ge0$ and $\sum_j w_j=1$ for each application track.
...
Shenzhen Zhongneng Tianyi Technology Co., Ltd. will provide application-level feedback on candidate prioritization criteria through two candidate-review checkpoints: one after formula-stage ranking and one after structure-hypothesis packaging. The partner will review the ranked BN-family candidate list and help assess which predicted materials families are most relevant to dielectric, insulating, and wide-band-gap scenarios. The project will provide the partner with a structured candidate-review package rather than unverified discovery claims: predicted properties, uncertainty estimates, domain-distance flags, novelty labels, validation notes, and a lightweight review demo.
